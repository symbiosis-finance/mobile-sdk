...
data class NerveStablePool(
    ...
    suspend fun getPoolRoute(client: SynthFabricContract): NetworkTokenPair = NetworkTokenPair(
        first = fromToken,
...
suspend fun NerveStablePool.getTargetTokenSynth(fromFabric: SynthFabricContract, targetFabric: SynthFabricContract): Erc20Token {
    require(
        setOf(fromFabric.network.chainId, targetFabric.network.chainId) ==
                setOf(fromNetwork.chainId, targetNetwork.chainId)
    ) {
